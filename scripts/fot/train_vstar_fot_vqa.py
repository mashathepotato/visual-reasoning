...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--mask-loss-weight", type=float, default=0.5)
    ...
    p.add_argument("--out", type=str, default="models/fot_vstar_heatmap_mcq.pt")
    return p.parse_args()
...
def collate_fn(batch, *, text_buckets: int, text_max_len: int) -> Dict[str, torch.Tensor]:
    ...
    masks = [b.get("target_mask") for b in batch]
    if all(m is not None for m in masks):
        target_mask = torch.stack([m for m in masks], dim=0)  # (B,1,H,W)
    else:
        target_mask = None
    ...
    for epoch in range(int(args.epochs)):
        ...
        for batch in train_loader:
            ...
            if "target_mask" in batch:
                ...
                loss = loss + float(args.mask_loss_weight) * F.binary_cross_entropy(heat, target)
        ...
        if val["acc"] > best_acc:
            ...
            torch.save({"model": model.state_dict(), "args": vars(args), "val": val}, str(out))
            print("Saved:", out)
